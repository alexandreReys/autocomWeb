<template>
  <nav class="navbar navbar-expand-md navbar-dark bg-dark">
    <router-link class="navbar-brand" :to="{ name: 'home' }">AutoCom Web</router-link>
    <button
    ...
    <div class="collapse navbar-collapse" id="navbarText" v-bind:class="{ show: show }">
      <ul class="navbar-nav mr-auto font-size12">
        ...
        >
          <a href="#" class="nav-link" @click="volta()">{{ route.meta.title }}</a>
        </router-link>
      </ul>
      <span class="navbar-text">
        <div v-if="user.name">
          <a class="nav-link pointer font-size12" @click="logout()">Olá, {{ user.name }} | Sair</a>
        </div>
      </span>
    </div>
  </nav>
